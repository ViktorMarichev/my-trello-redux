...
import { User as UserType } from '../types/tasks';
...
function Modal(props: ModalProps) {
  ...
  const onSubmit = (e: FormType) => {
    props.modalButtonHandler!(e.username!);
  };
  ...
  return (
    <ModalWrapper display={props.user!.username != null ? 'none' : 'block'}>
      <ModalBody>
        <Form
          onSubmit={onSubmit}
          validate={validate}
...
const mapDispatchToProps = (dispatch: (action: ActionType) => void) => {
  return {
    modalButtonHandler: (username: string) => {
      console.log(setUser);
      dispatch(
        setUser({
          username,
